...
public class Slack {
	...
	public Slack(GsConfiguration config) {
		...
		if (config.getSlackNotify() != null && !config.getSlackNotify().isEmpty()) {
			if (config.getSlackNotify().startsWith("https://")) {
				...
				if (tokenParts.length == 3) {
					notificationUrl = "https://hooks.slack.com/services/" + tokenParts[0] + "/" + tokenParts[1] + "/" + tokenParts[2];
				} else {
		...
		if (config.getSlackContact() != null && !config.getSlackContact().isEmpty()) {
			if (config.getSlackContact().startsWith("https://")) {
				...
				if (tokenParts.length == 3) {
					contactUrl = "https://hooks.slack.com/services/" + tokenParts[0] + "/" + tokenParts[1] + "/" + tokenParts[2];
				} else {
			...
			if (config.getMode().equals("PROD")) {
			    ...
			    try {
					...
					if (response.getStatusLine().getStatusCode() != 200) {
						LOG.error("Failed to successfully send message to Slack: {}", response.getStatusLine());
						return false;
					}
					return true;
				} catch (IOException e) {
	...
	private String getWebhookUrl(Channel channel) {
		...
		switch (channel) {
			case NOTIFY : return notificationUrl;
			case CONTACT: return contactUrl;
			default: return notificationUrl;
		}
	}
