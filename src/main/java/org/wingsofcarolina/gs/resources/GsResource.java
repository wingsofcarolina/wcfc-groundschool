...
public class GsResource {
	...
	public Response contact(Map<String, String> request) {
        Slack.instance().sendMessage(contactMessage(request));
	...
	private MessageRequest contactMessage(Map<String, String> request) {
		...
		ZonedDateTime time = LocalDateTime.now().atZone(zoneId);
		
		MessageRequest msg = MessageRequest.builder().username("Gs Contact")
				.iconEmoji(SlackMarkdown.EMOJI.decorate("books"))
				.channel("contact")
				.text("*Gs contact sent at : " + dateFormatGmt.format(time) + "*") // + SlackMarkdown.EMOJI.decorate("new"))
				.addAttachments(Attachment.builder()
						...
						.title(Title.builder()
						.text(email)
						.link(url("https://tailskid.planez.co"))
						.build())
						.footer(Footer.builder().text("Generated By Gs.co Server")
								.icon(url("https://platform.slack-edge.com/img/default_application_icon.png"))
								.timestamp(time.toEpochSecond()).build())
