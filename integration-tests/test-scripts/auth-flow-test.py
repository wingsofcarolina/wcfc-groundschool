...
def run_authentication_test():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Step 1: Navigate to private page (should redirect to login)
            print("Step 1: Navigating to /private...")
            
            try:
                response = page.goto("http://localhost:9305/private", wait_until="load", timeout=15000)
                print(f"Navigation response: {response.status}")
            except Exception as e:
                print(f"Navigation failed, trying /login directly: {e}")
                response = page.goto("http://localhost:9305/login", wait_until="load", timeout=15000)
            ...
            if "/login" in current_url:
                print("Still on login page, trying to navigate to private...")
                try:
                    page.goto("http://localhost:9305/private", wait_until="load", timeout=10000)
                except Exception as e:
                    print(f"Manual navigation failed: {e}")
            
            # Wait for private page to load
            wait_for_page_ready(page)
